...
function createWindow(title: string, width: number, height: number) {
  ...
  const metalView = BUILD_OS === "darwin"
    ? sdl2.symbols.SDL_Metal_CreateView(raw)
    : null;
...
function createRenderTarget(
  ...
  if (!context) {
    throw new Error("Failed to create WebGPU surface context");
  }

  return { canvas: new OffscreenCanvas(width, height), context };
}
//#endregion
...
const { window, metalView } = createWindow(
  "Deno + SDL2 + WebGPU",
  WIDTH,
  HEIGHT,
);
...
const { canvas, context: canvasContext } = createRenderTarget(
  surface,
  WIDTH,
  HEIGHT,
);
